    ...
    public class XMLHandler
    {
        ...
        public void ReadFile(string ReadFile)
        {
            if (File.Exists(ReadFile))
            {
                ...
                {
                    System.Windows.Forms.MessageBox.Show("Error Reading XML File: " + e.ToString());
                }
            }
        }
...
private List<string> CheckTypeRetList(EntryType Tin)
        {
            List<string> RetList = new List<string>();
            switch(Tin)
            {
                case EntryType.ExampleNode1:
                    RetList = X_Data.exampleList;
                    break;
                case EntryType.ExampleNode2:
                    RetList = X_Data.exampleList2;
                    break;
            }
            return RetList;
        }
